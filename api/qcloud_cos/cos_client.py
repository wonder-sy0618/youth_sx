...
from api.qcloud_cos.streambody import StreamBody
from api.qcloud_cos.xml2dict import Xml2Dict
from dicttoxml import dicttoxml
from api.qcloud_cos.cos_auth import CosS3Auth
from api.qcloud_cos.cos_comm import *
from api.qcloud_cos.cos_threadpool import SimpleThreadPool
from api.qcloud_cos.cos_exception import CosClientError
from api.qcloud_cos.cos_exception import CosServiceError
...
class CosS3Client(object):
    ...
    def get_auth(self, Method, Bucket, Key='', Expired=300, Headers={}, Params={}):
        ...
        """
        url = self._conf.uri(bucket=Bucket, path=parse.quote(Key, '/-_.~'))
    ...
    def put_object(self, Bucket, Body, Key, **kwargs):
        ...
        url = self._conf.uri(bucket=Bucket, path=parse.quote(Key, '/-_.~'))  # 提前对key做encode
    ...
    def get_presigned_download_url(self, Bucket, Key, Expired=300):
        ...
        """
        url = self._conf.uri(bucket=Bucket, path=parse.quote(Key, '/-_.~'))
        sign = self.get_auth(Method='PUT', Bucket=Bucket, Key=Key, Expired=300)
    ...
    def upload_part_copy(self, Bucket, Key, PartNumber, UploadId, CopySource, CopySourceRange='', **kwargs):
        ...
        url = self._conf.uri(bucket=Bucket, path=parse.quote(Key, '/-_.~')+"?partNumber={PartNumber}&uploadId={UploadId}".format(
    ...
    def create_multipart_upload(self, Bucket, Key, **kwargs):
        ...
        url = self._conf.uri(bucket=Bucket, path=parse.quote(Key, '/-_.~')+"?uploads")
    ...
    def complete_multipart_upload(self, Bucket, Key, UploadId, MultipartUpload={}, **kwargs):
        ...
        """
        headers = mapped(kwargs)
        url = self._conf.uri(bucket=Bucket, path=parse.quote(Key, '/-_.~')+"?uploadId={UploadId}".format(UploadId=UploadId))
    ...
    def put_object_acl(self, Bucket, Key, AccessControlPolicy={}, **kwargs):
        ...
        headers = mapped(kwargs)
        url = self._conf.uri(bucket=Bucket, path=parse.quote(Key, '/-_.~')+"?acl")
    ...
    def _inner_head_object(self, CopySource):
        ...
        url = self._conf.uri(bucket=bucket, path=parse.quote(path, '/-_.~'), scheme=self._conf._scheme, region=region)
        rt = self.send_request(
            method='HEAD',
            url=url,
    ...
    def append_object(self, Bucket, Key, Position, Data, **kwargs):
        ...
        url = self._conf.uri(bucket=Bucket, path=parse.quote(Key, '/-_.~')+"?append&position="+str(Position))
